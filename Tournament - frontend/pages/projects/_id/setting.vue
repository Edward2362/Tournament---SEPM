<template>
  <div class="project-content">
    ...
    <div>
      <input v-if="admin" type="button" @click="endProject()" value = "End Project!">
      <form>
          Reward: <input type="text" name="Reward" :placeholder= "[[reward]]" v-model ="reward"><br>
...
export default {
  ...
  methods:{
    async endProject(){
        axios.patch("/api/v1/projects/" + this.$route.params.id, {
        finished: true
      }).then(response => console.log(response.data.data))
    },
    async changeReward(){
      axios.patch("/api/v1/members/" + this.memberId, {
